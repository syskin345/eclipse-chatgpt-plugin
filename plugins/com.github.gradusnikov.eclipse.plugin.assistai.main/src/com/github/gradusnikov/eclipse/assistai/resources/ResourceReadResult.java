...
)
{
     ...
     * <p>
     * The slice is taken by the platform's line tracker and returned as the exact
     * characters between the two line boundaries, terminators included, so a CRLF
     * resource is not silently rewritten to LF on the way out.
     * <p>
     * This exists for the resources {@code readProjectResource} cannot reach: source
     * attached to a JAR has no project and no file, so a range of it can only be asked
     * for by type name. Without a range, the whole content is returned and this is
     * {@link #of} exactly.
     *
